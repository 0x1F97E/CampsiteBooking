...
using (var scope = app.Services.CreateScope())
{
    ...
    // Create AmenityLookups table if it doesn't exist
    try
    {
        Console.WriteLine("🔧 Creating AmenityLookups table if it doesn't exist...");

        var amenityLookupsTableExists = await db.Database.SqlQueryRaw<int>(
            "SELECT COUNT(*) as Value FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='CampsiteBookingDb' AND TABLE_NAME='AmenityLookups'"
        ).FirstOrDefaultAsync();

        if (amenityLookupsTableExists == 0)
        {
            await db.Database.ExecuteSqlRawAsync(@"
                CREATE TABLE AmenityLookups (
            ...
            ");
            Console.WriteLine("   ✅ AmenityLookups table created");
            ...
            ");
            Console.WriteLine("   ✅ 21 standard amenities inserted");
        }
        else
        {
            Console.WriteLine("   ✅ AmenityLookups table already exists");
        }
    }
    catch (Exception ex)
    {
        Console.WriteLine($"❌ Error creating AmenityLookups table: {ex.Message}");
        Console.WriteLine($"   Stack trace: {ex.StackTrace}");
    }

    // Create PurchaseOptions table if it doesn't exist
    try
    {
        Console.WriteLine("🔧 Creating PurchaseOptions table if it doesn't exist...");

        var purchaseOptionsTableExists = await db.Database.SqlQueryRaw<int>(
            "SELECT COUNT(*) as Value FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='CampsiteBookingDb' AND TABLE_NAME='PurchaseOptions'"
        ).FirstOrDefaultAsync();

        if (purchaseOptionsTableExists == 0)
        {
            await db.Database.ExecuteSqlRawAsync(@"
                CREATE TABLE PurchaseOptions (
                    Id INT AUTO_INCREMENT PRIMARY KEY,
                    ...
                    INDEX IX_PurchaseOptions_CampsiteId (CampsiteId),
                    INDEX IX_PurchaseOptions_AccommodationTypeId (AccommodationTypeId),
                    CONSTRAINT FK_PurchaseOptions_Campsites_CampsiteId FOREIGN KEY (CampsiteId) REFERENCES Campsites(Id) ON DELETE CASCADE,
                    CONSTRAINT FK_PurchaseOptions_AccommodationTypes_AccommodationTypeId FOREIGN KEY (AccommodationTypeId) REFERENCES AccommodationTypes(Id) ON DELETE SET NULL
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
            ");
            Console.WriteLine("   ✅ PurchaseOptions table created");
        }
        else
        {
            Console.WriteLine("   ✅ PurchaseOptions table already exists");
        }
    }
    catch (Exception ex)
    {
        Console.WriteLine($"❌ Error creating PurchaseOptions table: {ex.Message}");
        Console.WriteLine($"   Stack trace: {ex.StackTrace}");
    }
